...
describe('StorageService Database Operations', () => {
  ...
  describe('Database Connection Management', () => {
    ...
    test('should handle database connection failures gracefully', async () => {
      ...
      await expect(mockStorageService.getStorageStats()).rejects.toThrow('SQLITE_CANTOPEN');
    });
    ...
    test('should retry connection on temporary failures', async () => {
      ...
      await expect(mockStorageService.getStorageStats()).rejects.toThrow('SQLITE_BUSY');
    ...
    test('should handle corrupted database gracefully', async () => {
      ...
      await expect(mockStorageService.getViolationSummary()).rejects.toThrow('Data integrity');
    });
  });
  ...
  describe('Violation Storage Operations', () => {
    ...
    test('should handle duplicate violations appropriately', async () => {
      ...
      const result = await mockStorageService.storeViolations(duplicateViolations);
    ...
    test('should validate violation data before storage', async () => {
      ...
      const result = await mockStorageService.storeViolations(invalidViolations);
    ...
    test('should handle transaction failures during bulk insert', async () => {
      ...
      await expect(mockStorageService.storeViolations(largeViolationSet))
  ...
  describe('Violation Retrieval Operations', () => {
    ...
    test('should filter violations by criteria', async () => {
      ...
      mockStorageService.getViolationSummary.mockResolvedValue(filteredViolations);
  ...
  describe('Performance and Scalability', () => {
    ...
    test('should manage memory efficiently during large operations', async () => {
      ...
      try {
        ...
        expect(memoryAfter.heapUsed - memoryBefore.heapUsed).toBeLessThan(20 * 1024 * 1024); // <20MB
      } finally {
  ...
  describe('Error Recovery and Resilience', () => {
    test('should recover from temporary database locks', async () => {
      ...
      // First call should fail
      await expect(mockStorageService.storeViolations([createMockViolationRecord()]))
        .rejects.toThrow('SQLITE_BUSY');
      
      // Second call should succeed
      const result = await mockStorageService.storeViolations([createMockViolationRecord()]);
    ...
    test('should provide meaningful error messages for debugging', async () => {
      ...
      mockStorageService.storeViolations.mockRejectedValue(
        new Error('Database connection failed: Check database file permissions')
      );
      
      await expect(mockStorageService.storeViolations([]))
        .rejects.toThrow('Database connection failed: Check database file permissions');
    });
  });
});