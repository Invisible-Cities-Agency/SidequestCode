...
describe('PreferencesManager', () => {
  ...
  describe('Initialization and Defaults', () => {
    ...
    test('should load default preferences on first run', () => {
      ...
      expect(prefs.preferences.warnings.showTscEslintSeparationWarning).toBe(true);
  ...
  describe('File Persistence', () => {
    test('should save preferences to file', () => {
      ...
      expect(saved.preferences.analysis.defaultMode).toBe('warnings-and-errors');
    });
    ...
    test('should load existing preferences from file', () => {
      ...
      const testPrefs = {
        schemaVersion: '1.0.0',
        preferences: {
          analysis: { defaultMode: 'all', strictMode: true, includePatternChecking: true },
          warnings: { showTscEslintSeparationWarning: false, showPerformanceWarnings: true, showConfigurationHints: true },
          display: { colorScheme: 'dark', verboseOutput: true, showProgressIndicators: false },
          watch: { autoDetectConfigChanges: false, debounceMs: 1000, intervalMs: 5000 }
        },
        userChoices: {
  ...
  describe('Schema Migration', () => {
    test('should migrate incomplete preferences with defaults', () => {
      ...
      fs.writeFileSync(preferencesPath, JSON.stringify(incompletePrefs, null, 2));
      ...
      expect(migrated.preferences.warnings.showTscEslintSeparationWarning).toBe(true);
  ...
  describe('Custom Script Configuration', () => {
    ...
    test('should update custom script preferences', () => {
      ...
      expect(prefs.preferences.customTypeScriptScripts?.scriptTimeout).toBe(30_000);
    });
  });
  ...
  describe('Configuration State Queries', () => {
    test('should detect separation of concerns configuration', () => {
      ...
      preferencesManager.updateUserChoice('hasConfiguredSeparationOfConcerns', true);
      expect(preferencesManager.hasSeparationOfConcerns()).toBe(true);
    });
  });
