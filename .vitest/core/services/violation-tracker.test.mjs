...
describe("ViolationTracker", () => {
  ...
  describe("Violation Processing", () => {
    ...
    test("should deduplicate violations before processing", async () => {
      ...
      const storedViolations =
        mockStorageService.storeViolations.mock.calls[0][0];
    ...
    test("should validate violations and filter invalid ones", async () => {
      ...
      expect(
        result.errors.some((error) => error.includes("File path is required")),
      ).toBe(true);
    });
  ...
  describe("Hash Generation and Validation", () => {
    ...
    test("should validate violation hashes correctly", () => {
      ...
      expect(violationTracker.validateViolationHash(violation, hash)).toBe(
        true,
      );
      expect(
        violationTracker.validateViolationHash(violation, "invalid-hash"),
      ).toBe(false);
    });
  });
  ...
  describe("Violation Validation", () => {
    ...
    test("should validate line and column numbers", () => {
      ...
      const result2 = violationTracker.validateViolation(
        violationInvalidColumn,
      );
      ...
      expect(result2.errors).toContain(
        "Column number must be a non-negative integer",
      );
    });
  ...
  describe("Filtering Operations", () => {
    test("should filter violations by rule", () => {
      ...
      const filtered = violationTracker.filterViolationsByRule(violations, [
        "rule1",
      ]);
    ...
    test("should filter violations by severity", () => {
      ...
      const filtered = violationTracker.filterViolationsBySeverity(violations, [
        "error",
      ]);
    ...
    test("should filter violations by file", () => {
      ...
      const filtered = violationTracker.filterViolationsByFile(violations, [
        "/test/file1.ts",
      ]);
  ...
  describe("Batch Operations", () => {
    test("should process violations in batches", async () => {
      ...
      const results = await violationTracker.processBatchedViolations(
        violations,
        100,
      );
  ...
  describe("Error Handling", () => {
    test("should handle storage service errors gracefully", async () => {
      ...
      expect(result.errors.length).toBeGreaterThan(0);
      expect(
        result.errors.some((error) => error.includes("Storage error")),
      ).toBe(true);
    ...
    test("should handle performance metric recording errors", async () => {
      mockStorageService.recordPerformanceMetric.mockRejectedValue(
      ...
      await expect(
        violationTracker.processViolations(violations),
