...
describe('PRD Generation', () => {
  ...
  test('should generate comprehensive PRD content', async () => {
    ...
    expect(content).toContain('https://github.com/eyaltoledano/claude-task-master');
  });
  ...
  test('should set realistic targets in success metrics', async () => {
    const mockViolations = Array(100).fill(null).map(() => 
      createMockViolation({ category: 'test-category' })
    );
  ...
  test('should handle file write errors gracefully', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('❌ Failed to write PRD file: Error: Permission denied')
    );
