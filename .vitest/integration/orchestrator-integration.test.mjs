...
describe("Integration Tests - Full System Orchestration", () => {
  ...
  beforeEach(() => {
    ...
    testDirectory = fs.mkdtempSync(
      path.join(os.tmpdir(), "sidequest-integration-"),
    );
  ...
  describe("End-to-End Analysis Pipeline", () => {
    test("should complete full analysis workflow", async () => {
      ...
      expect(prefs.preferences.analysis.defaultMode).toBe(
        "warnings-and-errors",
      );
    ...
    test("should handle multi-stage violation processing", async () => {
      ...
      const result1 =
        await violationTracker.processViolations(stage1Violations);
      const result2 =
        await violationTracker.processViolations(stage2Violations);
    ...
    test("should handle service interdependencies", async () => {
      ...
      const adaptedStorage = new StorageService({
        enablePerformanceMetrics:
          prefs.preferences.analysis.defaultMode === "all",
      });
  ...
  describe("Performance Under Realistic Workloads", () => {
    test("should handle large-scale violation processing efficiently", async () => {
      ...
      const result =
        await violationTracker.processViolations(largeViolationSet);
  ...
  describe("Error Propagation and Recovery", () => {
    test("should handle cascading service failures gracefully", async () => {
      ...
      expect(
        result.errors.some((e) => e.includes("Database connection lost")),
      ).toBe(true);
  ...
  describe("Real-world Scenario Simulation", () => {
    test("should handle typical development workflow", async () => {
      ...
      const bugResult =
        await violationTracker.processViolations(buggyViolations);
      ...
      const resolvedCount =
        await violationTracker.markAsResolved(resolvedHashes);
