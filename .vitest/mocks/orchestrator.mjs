...
export const mockStorageService = {
  ...
  reset() {
    ...
    this.storeViolations.mockResolvedValue({ inserted: 1, updated: 0, errors: [] });
...
export const mockAnalysisService = {
  ...
  reset() {
    ...
    this.calculateViolationStats.mockResolvedValue(MOCK_CONSTANTS.STORAGE_STATS);
...
export const testScenarios = {
  ...
  storageError() {
    mockStorageService.reset();
    mockStorageService.storeViolations.mockRejectedValue(new Error('Storage failure'));
    mockStorageService.getDashboardData.mockRejectedValue(new Error('Database unavailable'));
  ...
  memoryPressure() {
    ...
    mockViolationTracker.processViolations.mockImplementation(async (violations) => {
