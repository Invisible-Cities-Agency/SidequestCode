  ...
  describe("Malformed Data Handling", () => {
    test("should handle violations with extreme edge case data", async () => {
      const mockStorage = {
        storeViolations: vi
          .fn()
          .mockResolvedValue({ inserted: 0, updated: 0, errors: [] }),
        recordPerformanceMetric: vi.fn().mockResolvedValue(),
      };
  ...
  describe("Concurrent Operation Edge Cases", () => {
    test("should handle concurrent validation cache access", () => {
      ...
      return Promise.all(promises).then((results) => {
        ...
        expect(results.every((r) => r.isValid === results[0].isValid)).toBe(
          true,
        );
    ...
    test("should handle preferences updates during concurrent reads", () => {
      ...
      const writePromises = Array.from({ length: 10 }, (_, i) =>
        Promise.resolve(
          manager.updateUserChoice("lastConfigUpdate", `update-${i}`),
        ),
      );

      return Promise.all([...readPromises, ...writePromises]).then(
        (results) => {
          // Reads should be consistent
          const preferences = results.slice(0, 20);
          expect(preferences.every((p) => p.schemaVersion === "1.0.0")).toBe(
            true,
          );
        },
      );
