...
export class CrossoverDetector {
  ...
  private detectTypeAwareRules(result: OrchestratorResult): void {
    const eslintViolations = result.violations.filter(v => v.source === 'eslint');
  ...
  private detectDuplicateViolations(result: OrchestratorResult): void {
    const tsViolations = result.violations.filter(v => v.source === 'typescript');
    const eslintViolations = result.violations.filter(v => v.source === 'eslint');

    const duplicates = new Map<string, { ts: Violation[], eslint: Violation[] }>();
  ...
  private detectConfigurationConflicts(result: OrchestratorResult): void {
    ...
    if (legacyTypeRules.length > 0) {
      ...
      this.warnings.push({
        ...
        suggestion: 'Move these checks to TypeScript compiler configuration (tsconfig.json) with strict settings: noImplicitAny, strictNullChecks, noImplicitReturns, etc.',
        severity: 'warn',
        affectedRules: rules
      });
    }
    ...
    if (eslintExecution > 10_000) { // > 10 seconds
      this.warnings.push({
        ...
        details: 'ESLint taking more than 10 seconds often indicates type-aware rules that require full TypeScript program analysis.',
        suggestion: 'Disable type-aware ESLint rules and focus on code quality rules (no-console, prefer-const, etc.) for faster feedback. Let TypeScript handle type checking.',
        severity: 'warn'
      });
    }
  }
  ...
  displayWarnings(): void {
    if (this.warnings.length === 0) {
      console.log('✅ No crossover issues detected - good separation of concerns!');
      return;
    }
 ...
 */
export function createCrossoverDetector(config?: Partial<CrossoverConfig>): CrossoverDetector {
