...
export interface IStorageService {
  ...
  getViolations(params?: ViolationQueryParams): Promise<Violation[]>;
  getViolationSummary(): Promise<any[]>;
  resolveViolations(hashes: string[]): Promise<number>;
  
  // Rule Check Management
  startRuleCheck(ruleId: string, engine: 'typescript' | 'eslint'): Promise<number>;
  completeRuleCheck(
    checkId: number, 
  ...
  getViolationHistory(params?: HistoryQueryParams): Promise<ViolationHistory[]>;
...
export interface IPollingService {
  ...
  // Rule Scheduling
  scheduleRule(ruleId: string, engine: 'typescript' | 'eslint', frequencyMs?: number): Promise<void>;
  unscheduleRule(ruleId: string, engine: 'typescript' | 'eslint'): Promise<void>;
  getScheduledRules(): Promise<RuleSchedule[]>;
  
  // Execution Control
  executeRule(ruleId: string, engine: 'typescript' | 'eslint'): Promise<RuleCheckResult>;
  ...
  // Events
  on(event: 'ruleStarted', listener: (ruleId: string, engine: string) => void): void;
  on(event: 'ruleCompleted', listener: (result: RuleCheckResult) => void): void;
  on(event: 'ruleFailed', listener: (ruleId: string, engine: string, error: Error) => void): void;
...
export interface IOrchestratorService {
  ...
  // Manual Operations
  runSingleCheck(ruleId: string, engine: 'typescript' | 'eslint'): Promise<RuleCheckResult>;
