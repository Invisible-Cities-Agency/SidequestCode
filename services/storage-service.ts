...
export class StorageService {
  ...
  async storeViolations(violations: OrchestratorViolation[]): Promise<{
    ...
    for (const violation of databaseViolations) {
      ...
      if (validationErrors.length > 0) {
        errors.push(
          `Violation ${violation.hash}: ${validationErrors.join(", ")}`,
        );
      } else {
          ...
          for (const violation of batch) {
            try {
              ...
              if (insertResult) {
                ...
                if (
                  existingViolation?.first_seen_at ===
              ...
              errors.push(
                `Failed to store violation ${violation.hash}: ${error}`,
              );
            }
          }
   ...
   */
  async getViolations(
    parameters: ViolationQueryParameters = {},
  ): Promise<Violation[]> {
    const database = getDatabase();
  ...
  async recordViolationDeltas(
    ...
    // Compute deltas
    const deltas = computeViolationDeltas(
      previousHashes,
  ...
  async getViolationHistory(
    parameters: HistoryQueryParameters = {},
  ...
  async upsertRuleSchedule(schedule: NewRuleSchedule): Promise<number> {
    ...
    const result = await database
      ...
      .onConflict((oc) =>
        oc.columns(["rule_id", "engine"]).doUpdateSet({
  ...
  async getDashboardData(): Promise<DashboardData> {
    ...
    const [
      ...
      database
        .selectFrom("violations")
        .select(sql<number>`COUNT(DISTINCT file_path)`.as("count"))
        .where("status", "=", "active")
        .executeTakeFirst(),
      database
        .selectFrom("rule_checks")
  ...
  async cleanupOldData(): Promise<{
    ...
    return {
      violationHistoryDeleted: historyResult.reduce(
        (sum, result) => sum + Number(result.numDeletedRows || 0),
        0,
      ),
      performanceMetricsDeleted: metricsResult.reduce(
        (sum, result) => sum + Number(result.numDeletedRows || 0),
        0,
      ),
      resolvedViolationsDeleted: violationsResult.reduce(
        (sum, result) => sum + Number(result.numDeletedRows || 0),
        0,
      ),
    };
  ...
  async getStorageStats(): Promise<{
    ...
    oldestViolation: string | undefined;
