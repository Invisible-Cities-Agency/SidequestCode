...
export class ConfigManager {
  ...
  async initializeDatabase(): Promise<Kysely<any>> {
    ...
    const databaseConfig: DatabaseConfig = {
      ...
      pragmas: {
        journal_mode:
          (this.config.database.enableWAL ?? true) ? "WAL" : "DELETE",
        synchronous: "NORMAL",
        cache_size: Math.floor(
          (this.config.performance.maxMemoryMB! * 1024 * 1024) / 1024,
        ), // Convert MB to KB
        foreign_keys: "ON",
        temp_store: "memory",
  ...
  async initializeServices(): Promise<{
    ...
    const storageConfig: StorageServiceConfig = {
      ...
      enablePerformanceMetrics:
        this.config.monitoring.enablePerformanceMetrics ?? true,
    };
  ...
  static async loadFromFile(configPath: string): Promise<ConfigManager> {
    try {
      ...
      const config = JSON.parse(
        configData,
      ) as Partial<OrchestratorServiceConfig>;
      return new ConfigManager(config);
    } catch (error) {
      console.warn(
        `[ConfigManager] Could not load config from ${configPath}, using defaults:`,
        error,
      );
  ...
  async saveToFile(configPath: string): Promise<void> {
    try {
      ...
      console.error(
        `[ConfigManager] Failed to save config to ${configPath}:`,
        error,
      );
      throw error;
    }
...
export function validateConfig(
  ...
  if (config.database?.path && config.database.path !== ":memory:") {
    ...
    if (!path.isAbsolute(databasePath) && !databasePath.startsWith("./")) {
      errors.push(
        "Database path must be absolute or relative (starting with ./)",
      );
    }
  }
  ...
  if (
    config.scheduling?.defaultFrequencyMs &&
